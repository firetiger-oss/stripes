...
func GoVendorModules(w io.Writer, r io.Reader, styles *Styles) {
...
func renderModulesTxtLine(w io.Writer, line string, styles *Styles) {
...
// preserved.
func renderModulesTxtModule(w io.Writer, s string, styles *Styles) {
	i := 0
	tokenIdx := 0
	sawArrow := false
	for i < len(s) {
