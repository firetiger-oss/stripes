...
// Parquet renders a parquet file as a styled lipgloss table, mirroring the
// look of [CSV]. Top-level schema fields become headers; each row is
// flattened to strings with fmt.Sprintf. Numeric columns are right-aligned.
//
// Parquet requires random access, so the entire input is buffered into
// memory before decoding.
func Parquet(w io.Writer, r io.Reader, styles *Styles) {
