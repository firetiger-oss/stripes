// Package txtar registers the txtar archive renderer with the stripes
// registry. Import for side effects to enable text/x-txtar support:
//
...
func writeTxtarMarker(w io.Writer, name string, styles *stripes.Styles) {
...
func renderTxtarComment(w io.Writer, comment string, styles *stripes.Styles) {
...
func renderTxtarFile(w io.Writer, f txtar.File, styles *stripes.Styles) {
	if len(f.Data) == 0 {
		io.WriteString(w, "\n")
		return
	}
	...
	renderer := stripes.Func(stripes.Detect(f.Name, peek), "")
	if renderer == nil {
...
func renderScriptLine(w io.Writer, line string, styles *stripes.Styles) {
...
func renderScriptTokens(w io.Writer, s string, styles *stripes.Styles) {
	first := true
	for i := 0; i < len(s); {
		c := s[i]
		if c == ' ' || c == '\t' {
...
// isNumber returns true when tok parses as a Go int, uint, or float —
// used to apply the Number style to numeric arguments in testscript
// commands.
