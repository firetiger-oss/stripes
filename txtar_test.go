...
content`,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			Txtar(&out, strings.NewReader(tt.input), DefaultStyles)
...
`
	var out strings.Builder
	Txtar(&out, strings.NewReader(input), DefaultStyles)
	full := out.String()
...
`
	...
	checks := []struct {
		...
		{"comment line", "# --color=always forces ANSI even when stdout is a pipe.", DefaultStyles.Comment},
		{"command keyword", "exec", DefaultStyles.Name},
		{"command keyword", "stdout", DefaultStyles.Name},
		{"flag stays plain", "--color=always", DefaultStyles.Text},
		{"short flag stays plain", "-p", DefaultStyles.Text},
		{"quoted string", `'\x1b\['`, DefaultStyles.String},
	}
...
`)
	...
	for b.Loop() {
		r.Reset(input)
		Txtar(io.Discard, r, DefaultStyles)
	}
}
